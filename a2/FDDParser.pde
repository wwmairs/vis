...
public class FDDParser {
  ...
  FDDParser(String filePath) {
    ...
    int numberOfNodes = Integer.parseInt(lines[0]);
    int numberOfEdges = Integer.parseInt(lines[numberOfNodes + 1]);
    ...
    for (int index = 1; index <= numberOfNodes; index++) {
      ...
      Node node = new Node(Float.parseFloat(nodeInfo[1]));
    ...
    for (int index = numberOfNodes + 2; index <= numberOfNodes + numberOfEdges + 1; index++) {
      ...
      defaultLength = Float.parseFloat(edgeInfo[2]);
